...
kbuf * kbuf_create(unsigned int elemsize) {
  kbuf * ret = NULL;
	...
	//1. Redondear elemsize a la siguiente potencia de 2
	...
	//    de items
	//2.2 Para tama�o mayor o igual a 4k, se necesita una pagina adicional
	//    para almacenar la estructura del buffer (paginas += 1)
	*/
	ret->pages=1;
	if(ret->size>=4096){
		ret->pages+=(ret->size /4096);
	}else{
		if(elemsize<4){
			ret->total = KBUF_SIZE / 4;	
		}else{
			ret->total = KBUF_SIZE / ret->size;			
		}
	}
	
	/*printf("\nLa cantidad de paginas usadas son: %d\n", ret->pages);*/
	...
	/*printf("\nTamanio kbuf: %d",sizeof(kbuf));
	printf("\nTamanio memoria reservada: %d", KBUF_SIZE);
	printf("\nCantidad de elementos que almacena el buffer: %d",ret->total);*/
	
	ret->free = ret->total - ocupados_kbuf;
	/*printf("\nCantidad de elementos ocupados por kbuf: %d", ocupados_kbuf);*/
